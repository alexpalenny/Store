...
namespace SeaStore.Repository.Common
{
  public static class ExpressionRewriter
  {
    ...
    /// <param name="inExpr">The in expr.</param>
    /// <returns></returns>
    public static Expression<Func<TOut>> CastParam<TIn, TOut>(Expression<Func<TIn>> inExpr)
    {
      if (inExpr.NodeType == ExpressionType.Lambda &&
          inExpr.Parameters.Count > 0)
      {
        ...
        return Expression.Lambda<Func<TOut>>(
                outBody,
                new ParameterExpression[] { outP });
      }
      else
    ...
    /// <param name="inExpr">The in expr.</param>
    /// <returns></returns>
    public static Expression<Func<TOut, dynamic>> CastParam<TIn, TOut>(Expression<Func<TIn, dynamic>> inExpr)
    {
      if (inExpr.NodeType == ExpressionType.Lambda &&
          inExpr.Parameters.Count > 0)
      {
        ...
        return Expression.Lambda<Func<TOut, dynamic>>(
                outBody,
                new ParameterExpression[] { outP });
      }
      else
    ...
    /// <param name="inExpr">The in expr.</param>
    /// <returns></returns>
    //public static Expression<Func<IQueryable<TOut>, IOrderedQueryable<TOut>>> CastParam<TIn, TOut>(Expression<Func<IQueryable<TIn>, IOrderedQueryable<TIn>>> inExpr)
    //{
    //    if (inExpr.NodeType == ExpressionType.Lambda &&
    //        inExpr.Parameters.Count > 0)
    //    {
    ...
    //            expr => (expr is ParameterExpression) ? outP : expr
    //        );
    //        return Expression.Lambda<Func<IQueryable<TOut>, IOrderedQueryable<TOut>>>(
    //                outBody,
    //                new ParameterExpression[] { outP });
    //    }
    //    else
